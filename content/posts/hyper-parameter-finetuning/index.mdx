...
The complete dataset contains 8294 images belonging to 4 different classes and each class has an approximately equal number of images, so it is a balanced dataset. For optimizing the network we shall only be using 500 random images from each class to train the network, and an additional 100 images per class to validate the performance of the model.