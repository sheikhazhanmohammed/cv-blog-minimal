...
As explained earlier, kernels are matrices used to extract features or information from an image, but the question is how does a kernel extract information. A kernel matrix is slid across an image, and each element of the kernel matrix is multiplied with the element of image matrix, the result of these multiplications are stored as a new image, which contains the extracted information. The animation below will help in understanding how kernels work.
![Kernel sliding over image](./kernelMultiplicationAnimation.gif)