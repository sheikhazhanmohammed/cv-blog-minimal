...
<img src="./kernelsMultiplicationWithImage.png" alt="Kernel multiplication with an image" width="600"/>
Kernel multiplication can be used to extract edges or other superficial features from an image. We will discuss about such kernels in further blogs.

A collection of numerous kernels together can extract various levels of features from the same input image. This process is known as convolving an image to get a intermediate feature space. Numerous kernels are bunched together in form of different channels, and the output of the operation is a multi-channel image. Mathematical representation of the same can be done in the following manner:
- An input image I of size HxW with single channel, so it is a simple grayscale image with height H and width W.
- M kernels having a size of 3x3, each kernel represented as k<sub>i</sub> where i ranges from 1 to M.
